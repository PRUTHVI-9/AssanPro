...
public class Measurement {
	...
	public static short qrs_time1 = 0,qrs_time2 = 0,qrs_time3 = 0,qrs_time4 = 0,sum_of_qrs_time = 0,RR_Interval,RR_Interval_QTc;
	...
	public void Calculate(Context context, short WriteLeadDataIndex, clsEcgScan ecgScan) {
		int i,j=0;
...
//		}
	...
	/*	for(i=0; i<MainActivity.SIZEOF_SAMPLE_BUFFER; i++) {
		...
		sCurrentIndex = WriteLeadDataIndex;
		for(i=0; i<MainActivity.SIZEOF_SAMPLE_BUFFER; i++) {
			switch(MainActivity.lead_no)
			{
				case 0: Sample = ecgScan.ECGLeadData0[sCurrentIndex];
					break;
				case 1: Sample = ecgScan.ECGLeadData1[sCurrentIndex];
					break;
				case 2: Sample = ecgScan.ECGLeadData2[sCurrentIndex];
					break;
				case 3: Sample = ecgScan.ECGLeadData3[sCurrentIndex];
					break;
				case 4: Sample = ecgScan.ECGLeadData4[sCurrentIndex];
					break;
				case 5: Sample = ecgScan.ECGLeadData5[sCurrentIndex];
					break;
				case 6: Sample = ecgScan.ECGLeadData6[sCurrentIndex];
					break;
				case 7: Sample = ecgScan.ECGLeadData7[sCurrentIndex];
					break;
				case 8: Sample = ecgScan.ECGLeadData8[sCurrentIndex];
					break;
				case 9: Sample = ecgScan.ECGLeadData9[sCurrentIndex];
					break;
				case 10: Sample = ecgScan.ECGLeadData10[sCurrentIndex];
					break;
				case 11: Sample = ecgScan.ECGLeadData11[sCurrentIndex];
					break;
			}
			sCurrentIndex = (short)(sCurrentIndex+1);
			if (sCurrentIndex > MainActivity.MAX_RECORD_BUFFER_INDEX )
				sCurrentIndex = 0;
...
//				Toast.LENGTH_LONG).show();
	 ...
	 public boolean QRSDetect_for_HR_calculation(Context context, short Current){
		...
		if(calculate_HR == true)
		{
			...
			RR_Interval_QTc = sum_of_qrs_time;//smk
...
//		Toast.LENGTH_LONG).show();
	...
	{ //r_index
		...
		short siRValue = 0;
		short siMaxValue = 0;
		short nMaxIndex = 0,p1,p2;
		short nHalfMaxBitcount = MAX_BITCOUNT/2;
		nStartIndex = (short) (nTriggerPoint - 10);
		siRValue = (short)Math.abs((median_buffer[nChannel_No][nStartIndex]- nHalfMaxBitcount));
		siMaxValue = siRValue;
		nMaxIndex = nStartIndex;
		while(nStartIndex < (nTriggerPoint + 10))
		{
			/********smk logic changed   for  -ve R peak 09-04-26*******/
			p1 = (short)Math.abs((median_buffer[nChannel_No][nStartIndex+1] - nHalfMaxBitcount));
			p2 = (short)Math.abs((median_buffer[nChannel_No][nStartIndex-1] - nHalfMaxBitcount));
			//if( (siRValue >= median_buffer[nChannel_No][nStartIndex+1]) && (siRValue >= median_buffer[nChannel_No][nStartIndex-1]) && (siRValue > 0))
			if( (siRValue >= p1) && (siRValue >= p2 ))
			{
				if(siRValue > siMaxValue)
				{
					siMaxValue = siRValue;
					nMaxIndex = nStartIndex;//commented earier
				}
				//commented by  smk  nMaxIndex = nStartIndex; //**modified on 03-05-2002
			}
			nStartIndex++;
			siRValue = (short)Math.abs((median_buffer[nChannel_No][nStartIndex]- nHalfMaxBitcount));
		}
...
//				Toast.LENGTH_LONG).show();
	...
	//					Toast.LENGTH_LONG).show();
		...
		{
			if(Math.abs(median_buffer[nChannel_No][nStartIndex] - siEValue) < (int)((float)siPValue * (float)0.45))//smk 0.35//vnp 3 feb 14 (abs(ReadSample(nStartIndex) - siEValue) < (int)((float)siPValue * (float)0.35))
				bStartFound = true;
			nStartIndex--;//vnp 3 feb 14 nStartIndex = nStartIndex - 2;
		}
	...
	{ //start and end of r wave
		...
		while((!bEndFound) && (nStartIndex < (nRPoint+15)))
		{
			if((median_buffer[nChannel_No][nStartIndex] >= siEValue) && (median_buffer[nChannel_No][nStartIndex+1]<= siEValue))//vnp 3 feb 14 ((siECGBuffer[nStartIndex] >= siEValue) && (siECGBuffer[nStartIndex+1] <= siEValue))
				bEndFound = true;
			/***smk added for Rend for -ve R peak***/
			if((median_buffer[nChannel_No][nStartIndex] <= siEValue) && (median_buffer[nChannel_No][nStartIndex+1]>= siEValue))
				bEndFound = true;
			nStartIndex++;
		}
		if(bEndFound)
		...
		else
		{
			...
			/***smk changed   for Rend for -ve R peak 09-04-26***/
			bEndFound = false;
			nStartIndex = nRPoint;
			short nHalfMaxBitcount = MAX_BITCOUNT/2;
			short siRValue = (short)(Math.abs(median_buffer[nChannel_No][nStartIndex]- nHalfMaxBitcount));//vnp 3 feb 14 siECGBuffer[nStartIndex];
			nREnd = nStartIndex;
			while(nStartIndex < (nRPoint+15))
			{
				nStartIndex++;
				p1 = (short)(Math.abs(median_buffer[nChannel_No][nStartIndex] - nHalfMaxBitcount));
				if(p1<  siRValue) //if(median_buffer[nChannel_No][nStartIndex]<  siRValue)//vnp 3 feb 14 (siECGBuffer[nStartIndex] <  siRValue)
					nREnd = nStartIndex;
	...
	{//start and end of t wave 
		...
		short siTValue = 0;
		short siEValue = 0;
		short UwaveslopeCnt = 0;
		nStartIndex = (short)(nSEnd + 2); //(nREnd + 20); smk //vnp 5 feb 14 nREnd + 40
		...
		bEndFound = false;	
		nTPoint = (short)(nSEnd+2);// (nREnd+20) smk//vnp 5 feb 14nREnd + 40;
		...
		if (nTPoint == nSEnd+2)//smk  ( nTPoint == nREnd)
		...
		//***TEnd detection algorithm modified on 30-01-26 smk  //31-01-2003
		while((bSlopeZero) && (nStartIndex > nTPoint))
		{
			if((short)(median_buffer[nChannel_No][nStartIndex-2] - median_buffer[nChannel_No][nStartIndex]) >= 2) //3 smk   //if((short)(median_buffer[nChannel_No][nStartIndex-1] - median_buffer[nChannel_No][nStartIndex]) >= 3)
		...
		{
			if((median_buffer[nChannel_No][nStartIndex] - median_buffer[nChannel_No][nStartIndex - 2]) >= 1)//2
			//vnp 7 feb 14((ReadSample(nStartIndex) - ReadSample(nStartIndex-2)) >= 2)
			{
				UwaveslopeCnt++;//smk for u wave  positive slope 30-01-26
				if (UwaveslopeCnt >= 3) {
					bSlopeZero = false;
					nStartIndex = (short)(nStartIndex-3);
				}
			}
			else
				UwaveslopeCnt--;
		...
		while((bSlopeZero) && (nStartIndex > nSEnd+2))//smk   nSEnd
		{
		...
		nStartIndex = (short)(nSEnd+2);//smk //nSEnd;
		bSlopeZero = true;

		while(bSlopeZero)
		{
			if(Math.abs(median_buffer[nChannel_No][nStartIndex] - median_buffer[nChannel_No][nStartIndex + 1]) <= 2)//smk  1
			//vnp 7 feb 14 (abs(ReadSample(nStartIndex) - ReadSample(nStartIndex+2)) <= 1)
				bSlopeZero = false;
			nStartIndex++; //vnp 7 feb 14 nStartIndex = nStartIndex + 2;
		}
	...
	void PutPositions_old(short nChannel_No) //vnp 27 may 16
	{
		...
		nWavePositions[nChannel_No][T_START	   ] = nTStart ;// //nWavePositions[nChannel_No][T_START	   ] = nTStart;
...
//				Toast.LENGTH_LONG).show();
	...
	static void Display_Interval_in_millisecs(short nChannel_No)
	{
		...
		double drr,dSampling,dT;
		drr = (double)RR_Interval_QTc;//smk //(double)RR_Interval;
		dSampling = (double)SAMPLING_FREQUENCY;
		//commented by smkdouble dT = Math.sqrt(drr / dSampling);//sqrt((double)drr/ (double)dSampling);
		if (current_HR_copy < 100)//changed formula depending on HR smk 13-01-26
			dT = Math.sqrt(drr / dSampling);//Bazett's formula//sqrt((double)drr/ (double)dSampling);
		else
			dT = Math.cbrt(drr / dSampling);//Fridericia's formula
		if(dT != 0)
...
// *******************P Axis calculations*************************/
		...
		if((fP_I_amp < 0) && (fP_aVF_amp > 0))
			nPAxis = 180 + nPAxis;
		else if((fP_I_amp < 0) && (fP_aVF_amp < 0)) {
			nPAxis = 180 - nPAxis;
			nPAxis = -nPAxis;
		}
		nAxis[P_AXIS] = nPAxis;
...
//			ldTanInv = tanl((long double)(fQRSAmpOfLeadAVF)/(long double)fQRSAmpOfLeadI);
		...
		if((fQRSAmpOfLeadI < 0) && (fQRSAmpOfLeadAVF > 0))//smk 09-04-26  for Right axis deviation //if((fQRSAmpOfLeadI < 0)
			nQRSAxis = 180 + nQRSAxis;
		else if((fQRSAmpOfLeadI < 0) && (fQRSAmpOfLeadAVF < 0))//smk 09-04-26  for extreme  deviation
		{
			nQRSAxis = 180 - nQRSAxis;
			nQRSAxis = -nQRSAxis;
		}
...
//		fTAmpOfLeadAVF = -0.07;
		...
		if((fTAmpOfLeadI < 0) && (fTAmpOfLeadAVF > 0) )
			nTAxis = 180 + nTAxis;
		else if((fTAmpOfLeadI < 0) && (fTAmpOfLeadAVF < 0) ) {
			nTAxis = 180 - nTAxis;
			nTAxis = -nTAxis;
		}
		nAxis[T_AXIS] = nTAxis;
